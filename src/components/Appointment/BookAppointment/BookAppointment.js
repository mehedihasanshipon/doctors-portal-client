...
const BookAppointment = ({selectedDate}) => {
    return (
        <section className="bookAppointment mb-5">
            <h2 style={{color:"#19D3B0"}} className="text-center mb-5">Available appointment on {selectedDate.toDateString()} </h2>
            <div className="container">
            <div className="row">
                {
                    bookingData.map(booking=> <BookingCard booking={booking} selectedDate={selectedDate} key={booking._id} /> )
                }
            </div>
            </div>
        </section>
